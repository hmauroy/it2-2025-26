...
def handle_click(kommando):
    """Setter dx og dy til å være i retning mot musa"""
    global xpos, ypos, dx, dy, canvas_width, canvas_height
    kode, event = kommando
    x_diff = event.x - xpos
    y_diff = event.y - ypos
    dx = x_diff / 10
...
canvas.bind("<Button-1>", lambda event: handle_click(("Høyreklikk",event)))


# Oppdater canvas før vi animerer
canvas.update()
